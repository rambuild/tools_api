module.exports = app => {
	...
	const fs = require('fs')
	// 封装百度OCR请求
	const { sendRequest: ocrRequest } = require('../utils/baiduAI')
	...
	app.post('/upload', upload.single('file'), async (req, res) => {
		...
		// 百度OCR请求
		const baiduOcrRes = await ocrRequest(classify, apiVersion, method, imgDataBase64, params)
		file.url = `https://api.fastro.cn/tools/static/upload/img/${file.filename}`
		file.status = 200
		res.send({
			file: {
				url: file.url,
